...
```
读取负片 (16-bit TIFF, 0-65535)
  → 反相 Invert (pixel = max - pixel)          # 负片转正像，橙红色罩仍在
  → 预校色预览：用非 VLM 算法做通道补偿 + 智能色阶，生成已白平衡的 8-bit 预览图
  → 把预览发给 VLM，VLM 返回 1~3 个中性灰区域的归一化中心坐标 [x, y]
  → 回反相原始图 (原分辨率, 0-65535)，按坐标做「自适应窗口取样」
     (边长 3~11，按「噪声 + 偏置」总误差最小自动选择)，取窗口均值作为参考 RGB
  → 由参考 RGB 算补偿比例 scale_R/G/B           # 把参考点三通道归一到同一亮度
  → 通道补偿 × scale
  → 智能色阶 (直方图百分位拉伸)
  → 暖调控制 (R↑+G↑ 黄色方向；绝不 R↑+B↑ 品红)
  → AI 偏色检测 + 修正 (反馈闭环)
  → 输出到原始位深
```

**位深说明：** 输入 16-bit (0-65535) 输出仍是 16-bit，输入 8-bit 才输出 8-bit。
发给 VLM 的是算法白平衡后的 8-bit 副本（仅用于识别位置），最终参考 RGB 始终从**原分辨率反相图**上取，不经过预览，因此不失精度。

**依赖提示：** LZW/Deflate 压缩的 16-bit TIFF 需要 `imagecodecs` 才能解码；
缺失时程序会明确报错并跳过该文件，而不是静默降为 8-bit（避免产出位深错误的结果）。
...
**色罩补偿的核心是找到一处「真实中性灰」作为参考。** 采样逐级回退：

策略 A — VLM 定位中性灰（已配置 API Key 时为首选）：
```
1. 用非 VLM 算法对反相图做预校色（通道补偿 + 智能色阶），
   生成一张已白平衡的 8-bit 预览图；
2. 把预览发给 VLM，VLM 返回 1~3 个中性灰区域的归一化中心坐标 [x, y]；
3. 回反相原始图（原分辨率，0-65535 尺度）上，对该坐标做自适应窗口取样
   （边长在 3~11 之间，按「噪声 + 偏置」总误差最小自动选择），
   取窗口均值作为参考 RGB。
```

策略 B — 边缘片基采样（VLM 失败 / 未配置 Key 时）：
```
取样区域：上/下/左/右四条边缘中「最均匀」的一侧（通常是未曝光片基）
